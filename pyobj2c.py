...
# Last updated: <2024/11/22 06:18:53 +0900>
"""
wavefront obj to c language array

usage: python pyobj2c.py [--nogamma] INPUT.obj

--nogamma : Gamma correction disabled
...
def main():
    ...
    parser.add_argument(
        "--nogamma", help="Gamma correction disabled", action="store_true"
    )
        ...
        with open(os.path.join(dirname, matfile), "r") as file:
            for line in file:
                ...
                if s[0] == "newmtl":
                    ...
                    if gamma_conv:
                        diff_r = linearrgb_to_srgb(float(s[1]))
                        diff_g = linearrgb_to_srgb(float(s[2]))
                        diff_b = linearrgb_to_srgb(float(s[3]))
                    else:
                        diff_r = float(s[1])
                        diff_g = float(s[2])
                        diff_b = float(s[3])
                    matdata[matname]["Kd"] = {"r": diff_r, "g": diff_g, "b": diff_b}
                elif s[0] == "Ka":
